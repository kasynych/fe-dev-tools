...
import { StepElement } from 'external/modules/demo-tools/step'
import Dom from 'external/modules/demo-tools/dom/dom'
...
export default abstract class StepForm {
  ...
  static getForm(highlightEl, values?: Partial<IFormValues>): string {
    let element
    if(values?.element) {
  ...
  static onXpathChange() {
    ...
    driver.highlight({
      ...
      popover: {
        ...
        description: StepForm.getForm(driver.getActiveStep(), formValues),
      },
    })
    driver.refresh()
    setTimeout(() => {
  ...
  static getSelectorTitle(element: Element) {
    const parentCnt = 10
    const parents = []
    const output = []
    for (let i = 0; i <= parentCnt - 1; i++) {
      const parent = dom.getNthParent(element as HTMLElement, i)
