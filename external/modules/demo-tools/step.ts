...
export default class Step implements IStepBase {
  ...
  get hasAmbiguousSelector(): boolean {
  ...
  get isElementBound() {
    return !isNil(this.selector) || !isNil(this.xPath) || !isNil((this as IHighlightingStep).area) || (!isNil(this.elementX) && !isNil(this.elementY)) || !isNil(this.element);
  }
  ...
  printElementBoundCheckErrors() {
    ...
    if (!isNil(this.selector) && !this.hasCorrectSelector) {
      ...
      console.table({
        'expected element selector': Dom.getSelector(this),
        'actual element selector': Dom.getSelector(document.querySelector(this.selector) as HTMLElement)
      })
    } else {
      if (!this.selector) {
    ...
    if (!isNil(this.elementX) && !isNil(this.elementY) && !this.hasCorrectCoordinates) {
      ...
      console.table({
        ...
        'actual coordinates': {x: Dom.getElement(this)?.getBoundingClientRect().x, y: Dom.getElement(this)?.getBoundingClientRect().y},
      })
    } else {
  ...
  adjustElementBoundStep(targetElement: HTMLElement = null) {
    if (!this.isElementBound) {
      return
    }

    let actualElement: HTMLElement | null = targetElement
    ...
    if (!actualElement && this.elementX && this.elementY) {
      actualElement = document.elementFromPoint(this.elementX, this.elementY) as HTMLElement
    }
...
export class StepElement implements Pick<HTMLElement, 'classList' | 'id' | 'localName'> {
  localName: string;
  id: string;
  classList: any;
