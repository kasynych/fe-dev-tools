...
import { clone, filter, find, findIndex, is, isEmpty, isNil, map, propEq, values } from 'ramda'
...
export default abstract class DemoTools {
  ...
  static async open(arg: IOptions | string = {} as IOptions) {
    ...
    title = title || DemoTools.gitSummary?.branch || 'default'
    ...
    DemoTools.demo = Demo.getInstance({
      ...
      ...newOptions,
      title,
      skipChecks: is(Boolean, newOptions.skipChecks) ? newOptions.skipChecks : Boolean(existingOptions.skipChecks),
    });
    ...
    setVar(DEMO_TOOLS_KEY, { ...demoToolsOptions, store: is(Boolean, newOptions.store) ? newOptions.store : true })

    if (newOptions.preloadDemo) {
      setVar(DEMO_TOOLS_KEY, { ...demoToolsOptions, preloadDemo: newOptions.preloadDemo })

      if (typeof newOptions.autostart !== 'undefined') {
        const demoToolsOptions = getVar(DEMO_TOOLS_KEY) || {};
        setVar(DEMO_TOOLS_KEY, { ...demoToolsOptions, autostart: newOptions.autostart })
      }
    }
  ...
  static async startOn(title: string) {
    ...
    const demo = find<IDemoOptions>((d: IDemoOptions) => String(d.title).includes(title), values(demos || []) as IDemoOptions[])

    if(demo) {
      await DemoTools.open({ ... demo, mode: 'compose', verbose: true })
      DemoTools.setDemoToolsOptions({ preloadDemo: demo?.title })
    } else {
      await DemoTools.open({ title, mode: 'compose', verbose: true })
