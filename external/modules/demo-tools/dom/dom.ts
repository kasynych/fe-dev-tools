import { equals, intersection, is, isEmpty, isNil, values } from 'ramda'
...
export default abstract class Dom {
  ...
  static getElement(arg: string | HTMLElement | IElementBoundStep) {
    ...
    if (!isNil((arg as IElementBoundStep).selector) && (arg as IElementBoundStep).hasCorrectSelector && !(arg as IElementBoundStep).hasAmbiguousSelector) {
      ret = document.querySelector((arg as IElementBoundStep).selector)
    }

    if (!ret && !isNil((arg as IElementBoundStep).xPath) && (arg as IElementBoundStep).hasCorrectXPath) {
      ret = getElementByXPath((arg as IElementBoundStep).xPath)
    }

    if (!ret && !isNil((arg as IElementBoundStep).elementX )&& !isNil((arg as IElementBoundStep).elementY) && (arg as IElementBoundStep).hasCorrectCoordinates) {
      ret = document.elementFromPoint(arg.elementX, arg.elementY)
    }

    if (!ret && !isNil((arg as IElementBoundStep).element) && is(HTMLElement, (arg as IElementBoundStep).element)) {
      ret = (arg as IElementBoundStep).element
    }
  ...
  static getSelector(arg: HTMLElement | IElementBoundStep) {
    if (isNil(arg)) {
      return null
    }
    ...
    if (arg instanceof HTMLElement) {
      ...
      element = (arg as IElementBoundStep).element
    }

    return element.localName
      + (element.id ? `#${element.id}` : '')
      + ( element.classList?.length ? `.${values(element.classList).join('.')}` : '' )
  }
}